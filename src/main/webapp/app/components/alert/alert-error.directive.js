(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('crowdwareApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-crowdwareApp-error');
                var entityKey = httpResponse.headers('X-crowdwareApp-params');
                if (errorHeader) {
